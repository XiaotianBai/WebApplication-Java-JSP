...
var topY=0,leftX=0; //自定义定位偏移量 2007-02-11 由 寒羽枫Add //选择日期 → 
//由 寒羽枫 2007-06-10 Add，通过 ID 来选日期 
...
//选择日期 → 由 寒羽枫 2006-06-25 Add 
function SelectDate(obj,strFormat,x,y) 
{ 

leftX =(x == null) ? leftX : x; 
topY =(y == null) ? topY : y;//自定义定位偏移量 2007-02-11 由 寒羽枫Add 
...
//cal = new Calendar(by, ey,1,strFormat);    //Initialize英文版，0 为中文版 
cal = (cal==null) ? new Calendar(by, ey, 0) : cal;    //不用每次都Initialize 2006-12-03 修正 
...
2006-06-25 由 寒羽枫 Edit为根据User指定的 style 来确定； 
*/ 
...
//将 onchange 转成其它函数，以免触发Validate事件 
...
, ev.lastIndexOf("}"));//去除Validate函数 ValidatorOnChange(); 
...
"sel_bg"        : "#FFCCCC", //已被选择的日期单元格背影色 2006-12-03 寒羽枫Add 
...
"tr_word"       : "#FFFFFF", //日历Head文字颜色 
"tr_bg"         : "#666666", //日历Head背影色 
...
"clear" : [["Clr"], ["CLS"]], 
"today" : [["Today"], ["TODAY"]], 
"pickTxt" : [["OK"], ["OK"]],//pickMode 精确到年、月时把今天变成“确定” 
"close" : [["Close"], ["CLOSE"]] 
} 
...
var cy = this.calendarYear;//2006-12-01 由寒羽枫Edit 
cy.length = 0; 
...
var cm = this.calendarMonth;//2006-12-01 由寒羽枫Edit 
cm.length = 0; 
for (var i = 0; i < 12; i++){ 
...
this.year = e.calendarYear.options[e.calendarYear.selectedIndex].value;//2006-12-01 由寒羽枫Edit 
...
//2006-12-30 由民工.砖家Edit 在Firefox 下年份错误 
...
if (dateArray[i] != "&nbsp;"){ 
tds[i].bgColorTxt = "td_bg_out"; //2009-03-03 Save背景色的 class 
var cur = new Date(); 
tds[i].isToday = false; 
...
if(calendar.DateMode == pickMode["day"]) //2009-03-03 当选择日期时，点击格子即Return值 
{ 
...
calendar.selectedDayTD = this; //2009-03-03 Record已选中的日子 
} 
} 
tds[i].style.cursor ="pointer"; //2007-08-06 由寒羽枫Add，鼠标变成手指状 
...
var now = new Date(); 
this.date = (dateObj.value.length > 0) ? new Date(dateObj.value.toDate(this.dateFormatStyle)) : now.format(this.dateFormatStyle).toDate(this.dateFormatStyle) ;//2008-01-29 寒羽枫Add → 若为空则根据dateFormatStyleInitialize日期 

if(this.panel.innerHTML==""||cal.dateFormatStyleOld != cal.dateFormatStyle)//2008-01-29 把构造表格放在此处，2009-03-03 若请示的样式改变，则重新Initialize 
{ 
this.draw(); 
this.bindYear(); 
this.bindMonth(); 
...
this.panel.style.left = (xy.x + leftX)+ "px"; //由寒羽枫 2007-02-11 Edit → 加入自定义偏移量 
this.panel.style.top = (xy.y + topY + dateObj.offsetHeight) + "px"; 

//由寒羽枫 2006-06-25 Edit → 把 visibility 变为 display，并Add失去焦点的事件 //this.setDisplayStyle("select", "hidden"); 
...
/* 已写在Return值的时候 ReturnDate 函数中，去除Validate事件的函数 
this.dateControl.changeEvent = this.dateControl.onchange;//将 onchange 转成其它函数，以免触发Validate事件 
this.dateControl.onchange = function() 
{if(typeof(this.changeEvent) =='function'){this.changeEvent();}}*/ 
if(this.dateControl.onblur != null){ 
this.dateControl.blurEvent = this.dateControl.onblur;}//2007-09-14 Save主文本框的 onblur ，使其原本的事件不被覆盖 
...
//焦点转移时隐藏日历 → 由寒羽枫 2006-06-25 Add 
Calendar.prototype.onblur = function() { 
if(!isFocus){this.hide();} 
} 

//以下由寒羽枫 2007-07-26 Edit → 确保日历容器节点在 body 最后，否则 FireFox 中不能出现在最上方 
function InitContainerPanel() //Initialize容器 
{ 
