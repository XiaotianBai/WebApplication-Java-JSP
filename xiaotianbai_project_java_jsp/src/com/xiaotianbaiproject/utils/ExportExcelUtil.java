package com.xiaotianbaiproject.utils;
...
public class ExportExcelUtil {
	...
	public void exportExcel(Collection<String[]> dataset, OutputStream out) {
		exportExcel("","测试POIExportEXCEL文档", null, dataset, out, "yyyy-MM-dd");
	}
	...
	public void exportExcel(String[] headers, Collection<String[]> dataset,
		...
		exportExcel("","测试POIExportEXCEL文档", headers, dataset, out, pattern);
	}

	/**
	 * 这是一个通用的方法，可以将放置在JAVA集合中并且符号一定 的数据以EXCEL 的形式输出到指定IO设备上
	 * 
	 * @param title
	 *            表格Title名
	 * @param headers
	 ...
	 *            需要Display的数据集合
	 * @param out
	 *            与输出设备关联的流Obj，可以将EXCEL文档Export到本地文件或者网络中
	 * @param pattern
	...
	public void exportExcel(String rootPath,String title, String[] headers,
		...
		// Set注释作者，当鼠标移动到单元格上是可以在状态栏中看到该Content.
		...
		while (it.hasNext()) {
			...
			for (short i = 0; i < t.length; i++) {
				...
				/*
				 * else if (value instanceof byte[]) { // 有图片时，Set行高为60px;
				 * row.setHeightInPoints(60); // Set图片所在列宽度为80px,注意这里单位的一个换算
				 * sheet.setColumnWidth(i, (short) (35.7 * 80)); //
				 ...
				 * HSSFWorkbook.PICTURE_TYPE_JPEG)); } else{ //其它数据Type都当作字符串简单处理
